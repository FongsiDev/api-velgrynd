...
loghandler = {
  ...
  error: {
    ...
    creator: `${creator}`,
    message:
      "An internal error occurred. Please report via WhatsApp wa.me/62895619083555",
  },
};
router.get("/canvas/welcome", async (req, res) => {
  ...
  if (!namagc)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter namagc",
    });
  if (!member)
    return res.json({
      status: false,
  ...
  res.sendFile(__path + "/database/welcome.png").catch((e) => {
...
router.get("/canvas/goodbye", async (req, res) => {
  ...
  res.sendFile(__path + "/database/goodbye.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/promote", async (req, res) => {
  ...
  if (!ppgc)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter ppgc",
    });
  if (!bg)
    return res.json({
      status: false,
  ...
  res.sendFile(__path + "/database/promote.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/demote", async (req, res) => {
  ...
  res.sendFile(__path + "/database/demote.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/level", async (req, res) => {
  ...
  if (!needxp)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter needxp",
    });
  if (!currxp)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter currxp",
    });
  if (!level)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter level",
    });
  if (!logorank)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter logorank",
    });
  ...
  res.sendFile(__path + "/database/rank.png").catch((e) => {
    res.json(loghandler.error);
  });
});
...
router.get("/canvas/levelup", async (req, res, next) => {
  ...
  var levelup = await new ch.Up().setAvatar(`${pp}`).toAttachment();
  data = levelup.toBuffer();
  await fs.writeFileSync(__path + "/database/sup.png", data);
  res.sendFile(__path + "/database/sup.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/gfx1", async (req, res) => {
  ...
  var gfx1 = await new ch.Gfx1().setName(`${nama}`).toAttachment();

  data = gfx1.toBuffer();
  await fs.writeFileSync(__path + "/database/gfx1.png", data);
  res.sendFile(__path + "/database/gfx1.png").catch((e) => {
    res.json(loghandler.error);
  });
});
...
router.get("/canvas/gfx2", async (req, res) => {
  ...
  var image = await new ch.Gfx2().setName(`${nama}`).toAttachment();

  data = image.toBuffer();
  await fs.writeFileSync(__path + "/database/gfx2.png", data);
  res.sendFile(__path + "/database/gfx2.png").catch((e) => {
    res.json(loghandler.error);
  });
});

router.get("/canvas/gfx3", async (req, res) => {
  (text1 = req.query.teks1), (text2 = req.query.teks2);

  if (!text1)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter teks1",
    });
  if (!text2)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter teks2",
    });
  ...
  res.sendFile(__path + "/database/gfx3.png").catch((e) => {
    res.json(loghandler.error);
  });
});
...
router.get("/canvas/gfx4", async (req, res) => {
  ...
  res.sendFile(__path + "/database/gfx4.png").catch((e) => {
    res.json(loghandler.error);
  });
});
...
router.get("/canvas/gfx5", async (req, res) => {
  ...
  var image = await new ch.Gfx5().setText(`${text}`).toAttachment();
...
router.get("/canvas/customgfx1", async (req, res) => {
  (text = req.query.teks), (bg = req.query.bg);
  if (!text)
    return res.json({
      status: false,
      creator: `${creator}`,
  ...
  var cgfx = await new ch.customGfx().setText(text).setBg(bg).toAttachment();

  data = cgfx.toBuffer();
  await fs.writeFileSync(__path + "/database/cgfx.png", data);
  res.sendFile(__path + "/database/cgfx.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/customgfx2", async (req, res) => {
  (text1 = req.query.teks1), (text2 = req.query.teks2), (bg = req.query.bg);
  if (!text1)
    return res.json({
      status: false,
      creator: `${creator}`,
  ...
  res.sendFile(__path + "/database/cgfx2.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/gura", async (req, res) => {
  ...
  var gura = await new ch.Gura().setName(teks).toAttachment();
  data = gura.toBuffer();
  await fs.writeFileSync(__path + "/database/Gura.png", data);
  res.sendFile(__path + "/database/Gura.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/spongebob", async (req, res) => {
  ...
  if (!image)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter image",
    });

  var burn = await new ch.Burn().setAvatar(image).toAttachment();

  data = burn.toBuffer();
  await fs.writeFileSync(__path + "/database/spongebob.png", data);
  res.sendFile(__path + "/database/spongebob.png").catch((e) => {
    res.json(loghandler.error);
  });
});
...
router.get("/canvas/patrick", async (req, res) => {
  ...
  var patrick = await new ch.Patrick().setAvatar(image).toAttachment();

  data = patrick.toBuffer();
  await fs.writeFileSync(__path + "/database/patrick.png", data);
  res.sendFile(__path + "/database/patrick.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/instagram", async (req, res) => {
  ...
  if (!post)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter post",
    });
  if (!followers)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter followers",
    });
  if (!following)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "masukan parameter following",
    });
  ...
  res.sendFile(__path + "/database/instagram.png").catch((e) => {
    res.json(loghandler.error);
  });
});
router.get("/canvas/xnxx", async (req, res) => {
  (pp = req.query.pp), (username = req.query.username);
  if (!pp)
    return res.json({
      status: false,
      creator: `${creator}`,
  ...
  var Xnxx = await new ch.Xnxx().setImage(pp).setTitle(username).toAttachment();
  data = Xnxx.toBuffer();
  await fs.writeFileSync(__path + "/database/xnxx.png", data);
  res.sendFile(__path + "/database/xnxx.png").catch((e) => {
    res.json(loghandler.error);
  });
});
// Downloader
...
router.get("/asupan", async (req, res) => {
  fetch(
    encodeURI(
      `https://raw.githubusercontent.com/binjaicity/warga62/master/asupan.js`
    )
  )
...
router.get("/ff", async (req, res) => {
  ...
  let result = await fetch("https://api.bayarlistrik.com/wp-json/cart/result", {
    headers: {
      ...
      "accept-language":
        "id-ID,id;q=0.9,ms-MY;q=0.8,ms;q=0.7,en-US;q=0.6,en;q=0.5",
